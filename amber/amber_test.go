...
func Benchmark_Amber_Parallel(b *testing.B) {
...
func Test_Render_Reentrant(t *testing.T) {
	...
	err = os.WriteFile(dir+"/child.amber", []byte("| C"), 0o600)
	require.NoError(t, err)
	err = os.WriteFile(dir+"/layout.amber", []byte("| L[#{embed()}]L"), 0o600)
	require.NoError(t, err)
	err = os.WriteFile(dir+"/page.amber", []byte("| X#{partial()}Y"), 0o600)
	require.NoError(t, err)
	...
	case err := <-done:
		require.NoError(t, err)
		// amber evaluates the interpolated call before streaming the line's
		// own text, so the embedded body lands ahead of the layout shell.
		require.Equal(t, "XCL[]LY", strings.ReplaceAll(trim(buf.String()), " ", ""))
...
func Test_Render_SelfEmbed_Errors(t *testing.T) {
	...
	err = os.WriteFile(dir+"/self.amber", []byte("| S#{embed()}S"), 0o600)
	require.NoError(t, err)
	err = os.WriteFile(dir+"/layout.amber", []byte("| L[#{embed()}]L"), 0o600)
	require.NoError(t, err)
...
func Test_AddFunc_Layout_Override(t *testing.T) {
	...
	err = os.WriteFile(dir+"/plain.amber", []byte("| A-#{embed()}-B"), 0o600)
	require.NoError(t, err)
	err = os.WriteFile(dir+"/child.amber", []byte("| C"), 0o600)
	require.NoError(t, err)
	err = os.WriteFile(dir+"/layout.amber", []byte("| L[#{embed()}]L"), 0o600)
...
func Test_Load_Retry(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(views+"/index.amber", []byte("| OK"), 0o600))
	require.NoError(t, engine.Load())
